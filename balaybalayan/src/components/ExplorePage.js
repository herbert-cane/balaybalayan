...
function ExplorePage() {
  ...
  // Navigate to the pageFile URL when "View" button is clicked
  const handleViewClick = (pageFile) => {
    console.log('Navigating to:', pageFile); // Debugging line to check the value of pageFile
    if (pageFile) {
      navigate(pageFile); // Navigate to the dormitory page
    } else {
      console.log('No pageFile found for this dorm.'); // Debugging line
    }
  };
          ...
          filteredDorms.map((dorm, index) => (
            <div key={index} className="explore-dorm-card">
              ...
              <button 
                ...
                onClick={() => handleViewClick(dorm.pageFile)}> {/* Handle click to navigate */}
                View
              </button>
            </div>
          ))}
